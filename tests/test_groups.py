...
def test_group_id(test_group_name, logged_rocket):
    _test_group_id = logged_rocket.groups_create(test_group_name).json().get('group').get('_id')
...
def test_groups_info(logged_rocket, test_group_name, test_group_id):
    groups_info_by_id = logged_rocket.groups_info(
    ...
    groups_info_by_name = logged_rocket.groups_info(
        room_name=test_group_name).json()
...
def test_groups_list_moderator(logged_rocket, test_group_name, test_group_id):
    groups_list_moderator = logged_rocket.groups_moderators(room_id=test_group_id).json()
    assert groups_list_moderator.get('success')
    groups_list_moderator_by_name = logged_rocket.groups_moderators(group=test_group_name).json()
...
def test_groups_add_and_remove_owner(logged_rocket, testuser_id, test_group_id):
    ...
    groups_add_owner = logged_rocket.groups_add_owner(test_group_id, user_id=testuser_id).json()
    assert groups_add_owner.get('success'), groups_add_owner.get('error')

    groups_remove_owner = logged_rocket.groups_remove_owner(test_group_id, user_id=testuser_id).json()
...
def test_groups_invite(logged_rocket, testuser_id, test_group_id):
    groups_invite = logged_rocket.groups_invite(
        test_group_id, testuser_id).json()
...
def test_groups_kick(logged_rocket, testuser_id):
    ...
    groups_invite = logged_rocket.groups_invite(
        id_group_created, testuser_id).json()
    assert groups_invite.get('success')
    groups_kick = logged_rocket.groups_kick(
        id_group_created, testuser_id).json()
...
def test_groups_leave(logged_rocket, test_group_id, testuser_id):
    ...
    name = str(uuid.uuid1())
    groups_create = logged_rocket.groups_create(name).json()
    logged_rocket.groups_invite(room_id=groups_create.get('group').get('_id'),
                                user_id=testuser_id)
    logged_rocket.groups_add_owner(groups_create.get('group').get('_id'),
                                   user_id=testuser_id).json()
...
def test_groups_rename(logged_rocket):
    ...
    groups_create = logged_rocket.groups_create(name).json()
    groups_rename = logged_rocket.groups_rename(room_id=groups_create.get('group').get('_id'),
                                                name=name2).json()
...
def test_groups_set_custom_fields(logged_rocket, test_group_id):
