...
def test_teams_add_update_remove_members(logged_rocket, test_team_id, testuser_id):
    ...
    assert teams_members.get("members")[1].get("user").get("_id") == testuser_id
    ...
    teams_members = logged_rocket.teams_members(team_id=test_team_id).json()
    assert "owner" in teams_members.get("members")[1].get("roles")
...
def test_teams_list_rooms(logged_rocket, test_team_id):
