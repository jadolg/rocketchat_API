...
def test_channels_list_moderator(logged_rocket):
    ...
    channel_name = logged_rocket.channels_info(room_id='GENERAL').json().get("channel").get("name")
...
def test_channels_add_and_remove_owner(logged_rocket, testuser_id):
    ...
    another_owner_id = logged_rocket.users_info(username='user1').json().get('user').get('_id')
    logged_rocket.channels_add_owner('GENERAL', user_id=another_owner_id).json()
    channels_remove_owner = logged_rocket.channels_remove_owner('GENERAL',
                                                                user_id=testuser_id).json()
...
def test_channels_rename(logged_rocket):
    ...
    channels_create = logged_rocket.channels_create(name).json()
    channels_rename = logged_rocket.channels_rename(room_id=channels_create.get('channel').get('_id'),
                                                    name=name2).json()
...
def test_channels_counters(logged_rocket):
    ...
    channels_counters_by_name = logged_rocket.channels_counters(room_name="general").json()
