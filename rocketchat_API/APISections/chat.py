...
class RocketChatChat(RocketChatBase):
    ...
    def chat_post_message(self, text, room_id, **kwargs):
        message = kwargs
        if room_id:
            message['rid'] = room_id
            if text:
            ...
            return api.call_api_post("chat.sendMessage",message=message)
        raise RocketMissingParamException("roomId required")
